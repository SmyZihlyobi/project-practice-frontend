...
export const FavoritesPage = observer(() => {
  const {
    favoriteProjectsList: favorites,
    getFavoriteProjects,
    getIsCacheLoaded,
    setPageSize,
  } = useProjectStore;
  ...
  useEffect(() => {
    // Устанавливаем бесконечную пагинацию для избранного
  ...
  if (!isAuthenticated) {
    return (
      <div className="h-[182px] w-full rounded-xl flex items-center justify-center text-lg">
        <div className="w-max">Авторизуйтесь для просмотра избранного</div>
      </div>
    );
  }
      ...
      {favorites.map(project => (
        <Card
          ...
          className={classNames(
            ...
            !project.active &&
              'before:absolute before:inset-0 before:bg-gradient-to-b before:from-red-500/10 before:to-transparent before:z-0 before:pointer-events-none',
          )}
        >
          ...
          <CardContent className="gap-2 flex flex-col">
            ...
            <Accordion type="multiple" className="w-full">
              <AccordionItem value="text">
                <AccordionTrigger disabled={false}>Описание проекта ↓</AccordionTrigger>
            ...
            {(project.technicalSpecifications || project.presentation) && (
              <div className="w-full flex flex-col gap-2 text-center sm:flex-row justify-between border-dashed border-2 p-3 rounded-xl">
                {project.technicalSpecifications && (
                  <a
                    className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 bg-primary text-primary-foreground shadow hover:bg-primary/90 h-9 px-4 py-2 sm:w-5/12 w-full sm:text-m"
